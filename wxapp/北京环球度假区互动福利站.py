...
#   WX_ID / bjhqbwz    可选，账号白名单（openid/wxid，& 或换行分隔），不填自动使用全部存活账号
...
# 账号列表：自动从 yyb_go 同步存活账号（可用 bjhqbwz / WX_ID 指定白名单）
ACCOUNT_REFS = yyb.resolve_accounts("bjhqbwz")
...
def main():
    ...
    if not accounts:
        print("❌ 未从 yyb_go 获取到存活账号(可配置环境变量 bjhqbwz 指定白名单), 退出。")
        return

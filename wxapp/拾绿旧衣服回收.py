#   --------------------------------注释区--------------------------------
#   https://sl.api.5tan.com/api/找authorization,格式：bearer 134561263326003496
#   变量:slhsck ，格式：多号@分割
#   token有效期不知道
#   corn: 每天跑一次就行 22 7 * * *
#   $ = new Env('石榴回收')
#   建议把请求头替换成自己的UA，填进ua里，不改也行挑选随机幸运儿封号
#   活动入口https://gitee.com/wry-fisher/push-notifications/blob/master/slhstgm.png
...
class yyf():
  ...
  'referer': "https://servicewechat.com/wxe417414e03e537aa/4/page-frame.html",
  'accept-language': "zh-CN,zh;q=0.9"
}

    def signin(self):
        url = "https://sl.api.5tan.com/api/signin/addSignIn"
        payload = {
        "platform": 1
        }
    ...
    def getmoney(self):
        url = "https://sl.api.5tan.com/api/user/getRec"
        params = {
        'tabDate': "4"
        }
    ...
    def cash(self,money=1):
        url = "https://sl.api.5tan.com/api/logmoney/cash"
        payload = {
        "money": str(money),
        "platform": 1
        }
...
if __name__ == '__main__':
    ...
    if not cookie:
        cookie = os.getenv("slhsck")
        if not cookie:
            print("请设置环境变量:slhsck")
            exit()

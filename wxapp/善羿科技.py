...
通过本目录下的 getCode.py 统一接口（牛子/应用宝双协议）使用 WX_ID
自动获取微信小程序登录 code，无需再手动抓包填入 SHANM_WX_CODE。

推荐环境变量：
  WX_ID           微信账号（多个用换行、& 或 | 分隔），格式：wxid#备注
                  示例：wxid_abc123#李四&wxid_xyz456
  WECHAT_SERVER   牛子协议服务地址（可选，默认在 getCode.py 中配置）
  YYB_SERVER      应用宝服务地址（可选）
  WX_APP_ID       小程序 AppID（可选，默认 wxc59eee06736849e8）
  定时任务：0 0 8,14,19 * * *

说明：
  登录成功后 token 会按 wxid 持久化到 .shanm_token.json，下次直接复用；
  若 token 鉴权失败会自动通过微信协议重新获取 code 登录。
"""

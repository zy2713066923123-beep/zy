...
class AutoTask:
    def __init__(self, script_name):
        ...
        self.wx_appid = "wx479a4a95ec031d79" # 微信小程序id
        self.log_msgs = []
        self.host = "taotaoka.languoyun.cn"
        self.nickname = ""
        self.token = ""
    ...
    def wxlogin(self, session, code):
        ...
        try:
            ...
            payload = f"code={code}&nickname=&headimg=&pid="
    ...
    def sign_in(self, session):
        ...
        """
        try:
            url = f"https://{self.host}/api/sign_add"
            response = session.post(url, timeout=5)
            response_json = response.json()
            if int(response_json['status']) == 1:
                self.log(f"[签到] 成功")

 ...
 *                        3) consumerToken
 * - WX_SERVER      yyb_go 协议服务地址（例如：http://127.0.0.1:18273）（可选，在 yyb.js 中配置）
...
async function runOne(line, idx) {
  ...
  const hasWechatServer = !!String(process.env.WX_SERVER || process.env.YYB_SERVER || process.env.WECHAT_SERVER || process.env.YINGYONGBAO_SERVER || '').trim();
...
(async () => {
  try {
    ...
    // 参考芳华未来.py：优先从 yyb 拉取全部存活账号（不受 WX_ID 过滤，配 3 条只跑 3 个）
    try {
    ...
    // 回退：WX_ID / 环境变量 / resolveAccounts
    if (!raw.trim()) {
      raw = process.env.WX_ID || process.env[ENV_NAME] || '';
    }
    if (!raw.trim()) {

...
#   WX_ID / wuying    可选，账号白名单（openid/wxid，& 或换行分隔），不填自动使用全部存活账号
...
# 账号列表：自动从 yyb_go 同步存活账号（可用 wuying / WX_ID 指定白名单）
ACCOUNT_REFS = yyb.resolve_accounts("wuying")
...
def main():
    ...
    if not entries:
        print("❌ 未从 yyb_go 获取到存活账号(可配置环境变量 wuying 指定白名单), 退出。")
        return

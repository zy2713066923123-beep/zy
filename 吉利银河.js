...
class UserInfo {
    ...
    async refresh_token() {
        try {
            ...
            const token = result.data && result.data.centerTokenDto && result.data.centerTokenDto.token;
            if (result.code == 'success' && token) {
...
// ==================== API ==================== // 
// 最大尝试次数与重试间隔（可用环境变量覆盖）
const HTTP_MAX_TRY = Math.max(1, Number(process.env.jlyh_retry || 3));
const HTTP_RETRY_DELAY = Math.max(500, Number(process.env.jlyh_retry_delay || 3000));
...
function httpSend(options, method) {
    return new Promise((resolve) => {
        $[method](options, (err, resp, data) => resolve({ err, resp, data }));
    });
}

async function httpRequest(options, method = null) {
    method = options.method ? options.method.toLowerCase() : options.body ? "post" : "get";
    let lastMsg = '未知错误';

    for (let i = 1; i <= HTTP_MAX_TRY; i++) {
        const { err, resp, data } = await httpSend(options, method);
        const body = parseBody(data);
        const status = resp && (resp.statusCode || resp.status);

        // 服务端已下发业务数据（即使 HTTP 状态非 2xx，如网关 400/401 也会带 JSON），直接交给业务层判断，不再重试
        if (body && typeof body === 'object' && ('code' in body || 'message' in body)) return body;
        // 无网络错误且拿到了可用内容
        if (!err && body !== undefined) return body;

        lastMsg = err ? (err.message || String(err)) : (data ? '响应内容无法解析' : '返回数据为空');
        console.log(`⚠️ ${method.toUpperCase()} 第 ${i}/${HTTP_MAX_TRY} 次请求失败${status ? `(HTTP ${status})` : ''}: ${lastMsg}`);

        if (i < HTTP_MAX_TRY) {
            const wait = HTTP_RETRY_DELAY * i; // 线性退避
            console.log(`⏳ ${Math.round(wait / 1000)} 秒后重试...`);
            await $.wait(wait);
        }
    }

    // 始终返回对象，避免调用方读取 result.code 时抛 TypeError
    return { code: -1, message: `请求失败(已重试 ${HTTP_MAX_TRY} 次): ${lastMsg}` };
}
